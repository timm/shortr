...
Explore N points via  O(log2(N)) evaluations. In pass1, find
and eval  two distant points using multi-objective criteria.
...
-- than their combination. Return nil otherwise
function RANGE.merge(i,j,min,        a,b,c)
  a,b,c = i.ys, j.ys, i.ys:merge(j.ys)
  if a.n<min or b.n<min or c:div() <= (a.n*a:div()+b.n*b:div())/c.n then 
    return RANGE(a.xlo, b.xhi, c)  end end
   
-- ## Discretization (making ranges)

-- RANGES are generated by two class methods kept in `RANGE.fun`.
RANGE.fun = {}

-- __.fun.ranges(  `rows`  :{ROW},  `xcol`  :(NUM|SYM),<br>`yclass`:KLASS,  `y`  :function  )  :table__  
-- Returns a list `{ranges={RANGE},  div=number}` that scores each range.  
-- Numeric ranges are first divided into groups of size `(hi-lo)/THE.bins` 
-- then subsequently, adjacent ranges are merged (see `.fun.merge`). 
...
function RANGE.fun.ranges(rows,xcol,yklass,y)
  ...
  list = xcol.is=="NUM" and RANGE.fun.merge(list, n^THE.min) or list
  return {ranges= list,
...
-- Finally, for numerics, expand the ranges to cover everything from minus to plus
-- infinity.
function RANGE.fun.merge(b4,min)
  local j,t a,b,c = 1,{}
  while j <= #b4 do 
    a, b = b4[j], b4[j+1]
    if b then c = a:merge(b,min)
              if c then a,j = c,j+1 end end -- skip one, since it has been merged
    t[#t+1] = a
    j = j + 1 end
