_=require"lib"
local big,fmt,help,lint=_.big,_.fmt,_.help,_.lint
local R,thing=_.R,_.thing
...
function bin(i,...) return i.is.bin(i,...) end
function dist(i,...) return i.is.dist(i,...) end
function div(i,...)  return i.is.div(i,...) end
function like(i,...) return i.is.like(i,...) end
function mid(i,...)  return i.is.mid(i,...) end
function ok(i,...)  return i.is.ok(i,...) end
function say(i,...)  return i.is.say(i,...) end

local function cols(of,t,txt)
  local w= (txt or ""):find"-$" and -1 or 1
  return {_of=of,at=at or 0, txt=txt or {}} end

local function col(of,all) return {_of=of,_all=all,i.kept={}, i.n=0} end

local Syms,Sym={}
function Syms.NEW(at,txt) return cols(SYMS, at,txt)  end
function Sym.NEW(all)     local i=col(Sym,all); i.mode=nil;i.most=-1,return i end

local Nums,Num= {}
function Nums.NEW(at,txt,i) 
  i=Syms._BASE(at,txt); 
  i.of, i.w =Nums; (txt or ""):find"-$" and -1 or 1}
  return i end

local Some={}
function Some.NEW() return {of=Some, i.max=the.some, ok=true, kept={}}
function Some.add(i,x,n)
   for _ =1,(n or 1) do:
     if     #i.kept < i.nax then i.ok=false;push(i.kept,x) 
     elseif R() < i.nums/i.n then i.ok=false;i.kept[R(#i.kept)]=x end end end 
function Some.has(i)
  if not i.ok then table.sort(i.kept); i.ok=true; return i.ok end end

function Col.NEW(at,txt) return {of=Of.NEW(at,txt),kept={},some=the.some
-------------------------------------------------------------------------------
-- ## Names
