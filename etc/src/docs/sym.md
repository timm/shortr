...
> ***SYM(`at` :?int, `txt` :?str) :SYM*** :point_right:  Summarize a stream of non-numerics.
...
> ***add(`i` :`SYM` : `x` :any, `n` :?int=1)*** :point_right:  Add `n` count to `i.kept[n]` .
...
> ***bin(`i` :`SYM` : `x` :any)*** :point_right:  return `x` mapped to a finite range (just return x)
...
> ***clone(`i` :SYM) :SYM*** :point_right:  Return a class of the same structure.
...
> ***like(`i` :SYN,`x` :any,`prior` :num):num*** :point_right:  return how much `x` might belong to `i`.
...
> ***merge(`i` :SYM,`j` :SYM):SYM*** :point_right:  combine two symbols
...
> ***merge(`i` :SYM,`t` :tab):tab*** :point_right:  merge a list of bins (for symbolic y-values)
...
> ***mid(`i` :SYM):tab*** :point_right:  Return a columns' `mid`ddle (central tendency).
...
> ***div(`i` :SYM):tab*** :point_right:  Return `div`ersity of a column
