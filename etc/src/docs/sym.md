...
> ***SYM(`at` :?int, `txt` :?str) :SYM*** :bangbang:  Summarize a stream of non-numerics.
...
> ***add(`i` :`SYM` : `x` :any, `n` :?int=1)*** :bangbang:  Add `n` count to `i.kept[n]` .
...
> ***bin(`i` :`SYM` : `x` :any)*** :bangbang:  return `x` mapped to a finite range (just return x)
...
> ***clone(`i` :SYM) :SYM*** :bangbang:  Return a class of the same structure.
...
> ***like(`i` :SYN,`x` :any,`prior` :num):num*** :bangbang:  return how much `x` might belong to `i`.
...
> ***merge(`i` :SYM,`j` :SYM):SYM*** :bangbang:  combine two symbols
...
> ***merge(`i` :SYM,`t` :tab):tab*** :bangbang:  merge a list of bins (for symbolic y-values)
...
> ***mid(`i` :SYM):tab*** :bangbang:  Return a columns' `mid`ddle (central tendency).
...
> ***div(`i` :SYM):tab*** :bangbang:  Return `div`ersity of a column
