...
> ***SYM(`at` :?int, `txt` :?str) :SYM*** :mag:  Summarize a stream of non-numerics.
...
> ***add(`i` :`SYM` : `x` :any, `n` :?int=1)*** :mag:  Add `n` count to `i.kept[n]` .
...
> ***bin(`i` :`SYM` : `x` :any)*** :mag:  return `x` mapped to a finite range (just return x)
...
> ***clone(`i` :SYM) :SYM*** :mag:  Return a class of the same structure.
...
> ***like(`i` :SYN,`x` :any,`prior` :num):num*** :mag:  return how much `x` might belong to `i`.
...
> ***merge(`i` :SYM,`j` :SYM):SYM*** :mag:  combine two symbols
...
> ***merge(`i` :SYM,`t` :tab):tab*** :mag:  merge a list of bins (for symbolic y-values)
...
> ***mid(`i` :SYM):tab*** :mag:  Return a columns' `mid`ddle (central tendency).
...
> ***div(`i` :SYM):tab*** :mag:  Return `div`ersity of a column
