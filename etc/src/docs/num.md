...
> ***NUM(`at` :?int, `txt` :?str) :NUM*** :large_blue_circle:  Summarize a stream of numbers.
...
> ***add(`i` :`NUM` : `x` :num, `n` :?int=1)*** :large_blue_circle:  `n` times,update `i`'s SOME object.
...
> ***bin(`i` :`NUM` : `x` :any)*** :large_blue_circle:  return `x` mapped to a finite range
...
> ***clone(`i` :(SYM|NUM)) :(SYM|NUM)*** :large_blue_circle:  Return a class of the same structure.
...
> ***div(`i` :NUM) :tab*** :large_blue_circle:  Return `div`ersity of a column
...
> ***like(`i` :NUM, `x` :any)*** :large_blue_circle:  Return the likelihood that `x` belongs to `i`.
...
> ***merge(`i` :NUM,`j` :NUM) :NUM*** :large_blue_circle:  combine two numerics
...
> ***merge(`i` :NUM,`t` :[BIN]) :[BIN]*** :large_blue_circle:  merge a list of bins (for numeric y-values)
...
> ***mid(`i` :NUM)) :tab*** :large_blue_circle:  Return a columns' `mid`ddle
...
> ***norm(`i` :NUM, `x` :num) :num*** :large_blue_circle:  Normalize `x` 0..1 for lo..hi,
