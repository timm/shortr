...
> ***NUM(`at` :?int, `txt` :?str) :NUM*** :bangbang:  Summarize a stream of numbers.
...
> ***add(`i` :`NUM` : `x` :num, `n` :?int=1)*** :bangbang:  `n` times,update `i`'s SOME object.
...
> ***bin(`i` :`NUM` : `x` :any)*** :bangbang:  return `x` mapped to a finite range
...
> ***clone(`i` :(SYM|NUM)) :(SYM|NUM)*** :bangbang:  Return a class of the same structure.
...
> ***div(`i` :NUM) :tab*** :bangbang:  Return `div`ersity of a column
...
> ***like(`i` :NUM, `x` :any)*** :bangbang:  Return the likelihood that `x` belongs to `i`.
...
> ***merge(`i` :NUM,`j` :NUM) :NUM*** :bangbang:  combine two numerics
...
> ***merge(`i` :NUM,`t` :[BIN]) :[BIN]*** :bangbang:  merge a list of bins (for numeric y-values)
...
> ***mid(`i` :NUM)) :tab*** :bangbang:  Return a columns' `mid`ddle
...
> ***norm(`i` :NUM, `x` :num) :num*** :bangbang:  Normalize `x` 0..1 for lo..hi,
