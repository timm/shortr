...
> ***NUM(`at` :?int, `txt` :?str) :NUM*** :arrow_right:  Summarize a stream of numbers.
...
> ***add(`i` :`NUM` : `x` :num, `n` :?int=1)*** :arrow_right:  `n` times,update `i`'s SOME object.
...
> ***bin(`i` :`NUM` : `x` :any)*** :arrow_right:  return `x` mapped to a finite range
...
> ***clone(`i` :(SYM|NUM)) :(SYM|NUM)*** :arrow_right:  Return a class of the same structure.
...
> ***div(`i` :NUM) :tab*** :arrow_right:  Return `div`ersity of a column
...
> ***like(`i` :NUM, `x` :any)*** :arrow_right:  Return the likelihood that `x` belongs to `i`.
...
> ***merge(`i` :NUM,`j` :NUM) :NUM*** :arrow_right:  combine two numerics
...
> ***merge(`i` :NUM,`t` :[BIN]) :[BIN]*** :arrow_right:  merge a list of bins (for numeric y-values)
...
> ***mid(`i` :NUM)) :tab*** :arrow_right:  Return a columns' `mid`ddle
...
> ***norm(`i` :NUM, `x` :num) :num*** :arrow_right:  Normalize `x` 0..1 for lo..hi,
