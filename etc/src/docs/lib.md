...
> ***rogues()*** :mag:  Find rogue locals. Run `rogues()` _last_ after everything else.
...
> ***lt(`x` :str):function*** :mag:  
> ***gt(`x` :str):function*** :mag:  Returns functions that sorts on `x`
...
> ***same(`x` :any):any*** :mag:  Return x, unchanged.
...
> ***`big` :num*** :mag:  return a big number
...
> ***R(`max` :?num=1):num*** :mag:  return a random number `0..max`.
...
> ***rnd(`x` :num, `places` :int):num*** :mag:  return `x` rounded to some number of `places`.
...
> ***rnds(`t` :num, `places` :?int=2):t*** :mag:  return items in `t` rounds to `places`.
...
> ***splice(`t` :tab,start=?int=1,`stop` :?num=#t,`step` :?num=1):t*** :mag:  pull items
...
> ***sort(`t` :tab, `f` :fun) :tab*** :mag:  Return `t`, sorted of function `f` (default "<").
...
> ***push(`t` :tab, `x` :any) :x*** :mag:  Add `x` to end of `t`; return `t`.
...
> ***per(`t` :tab, `p` :?float=.5) :x*** :mag:  Return `p`-th ranked item from `t`.
...
> ***map(`t` :tab, `f` :fun): tab*** :mag:  
> ***kap(`t` :tab, `f` :fun): tab*** :mag:  
> ***maps(`list1` :tab, `list2` :tab, `f` :fun): tab*** :mag:  
> ***kaps(`list1` :tab, `list2` :tab, `f` :fun): tab*** :mag:  Return items in `t`, filtered thru `f`.
...
> ***thing(`s` :str):any*** :mag:  Coerce string to whatever
...
> ***words(`s` :str, `sep` :str, `fun` :fun):tab*** :mag:  Return `t` filled with `s`, split  on `sep`.
...
> ***lines(`file` :str,  `fun` :fun):tab*** :mag:  Call `fun` with lines
...
> ***csv(`file` :str,  `fun` :fun):tab*** :mag:  Call `fun` with lines, split on ",", 
...
> ***fmt(`s` :str,...) :str*** :mag:  emulate prinft
...
> ***cat(`t` :tab):str*** :mag:  Return table as string. For key-indexed lists, show keys (sorted).
...
> ***chat(`t` :tab):t*** :mag:  Print table (as string). Return `t`.
...
> ***chunks(`file` :str)*** :mag:  divide source code into comments and code.
...
> ***opts(`x` :str) :tab*** :mag:  Parse `str` for lines with `--`; then pull keys+defaults. 
...
> ***cli(`t` :tab) :tab*** :mag:  For keys in `t`, look for updates on command-line. 
...
> ***on(`opts` :tab, `tests` :[fun])*** :mag:  Run some tests.
...
> ***obj(`name` :str, `fun` :fun):object*** :mag:  Return a klass `name` with constructor `fun`.
