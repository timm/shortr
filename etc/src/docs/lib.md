...
> ***rogues()*** :large_blue_circle:  Find rogue locals. Run `rogues()` _last_ after everything else.
...
> ***lt(`x` :str):function*** :large_blue_circle:  
> ***gt(`x` :str):function*** :large_blue_circle:  Returns functions that sorts on `x`
...
> ***same(`x` :any):any*** :large_blue_circle:  Return x, unchanged.
...
> ***`big` :num*** :large_blue_circle:  return a big number
...
> ***R(`max` :?num=1):num*** :large_blue_circle:  return a random number `0..max`.
...
> ***rnd(`x` :num, `places` :int):num*** :large_blue_circle:  return `x` rounded to some number of `places`.
...
> ***rnds(`t` :num, `places` :?int=2):t*** :large_blue_circle:  return items in `t` rounds to `places`.
...
> ***splice(`t` :tab,start=?int=1,`stop` :?num=#t,`step` :?num=1):t*** :large_blue_circle:  pull items
...
> ***sort(`t` :tab, `f` :fun) :tab*** :large_blue_circle:  Return `t`, sorted of function `f` (default "<").
...
> ***push(`t` :tab, `x` :any) :x*** :large_blue_circle:  Add `x` to end of `t`; return `t`.
...
> ***per(`t` :tab, `p` :?float=.5) :x*** :large_blue_circle:  Return `p`-th ranked item from `t`.
...
> ***map(`t` :tab, `f` :fun): tab*** :large_blue_circle:  
> ***kap(`t` :tab, `f` :fun): tab*** :large_blue_circle:  
> ***maps(`list1` :tab, `list2` :tab, `f` :fun): tab*** :large_blue_circle:  
> ***kaps(`list1` :tab, `list2` :tab, `f` :fun): tab*** :large_blue_circle:  Return items in `t`, filtered thru `f`.
...
> ***thing(`s` :str):any*** :large_blue_circle:  Coerce string to whatever
...
> ***words(`s` :str, `sep` :str, `fun` :fun):tab*** :large_blue_circle:  Return `t` filled with `s`, split  on `sep`.
...
> ***lines(`file` :str,  `fun` :fun):tab*** :large_blue_circle:  Call `fun` with lines
...
> ***csv(`file` :str,  `fun` :fun):tab*** :large_blue_circle:  Call `fun` with lines, split on ",", 
...
> ***fmt(`s` :str,...) :str*** :large_blue_circle:  emulate prinft
...
> ***cat(`t` :tab):str*** :large_blue_circle:  Return table as string. For key-indexed lists, show keys (sorted).
...
> ***chat(`t` :tab):t*** :large_blue_circle:  Print table (as string). Return `t`.
...
> ***chunks(`file` :str)*** :large_blue_circle:  divide source code into comments and code.
...
> ***opts(`x` :str) :tab*** :large_blue_circle:  Parse `str` for lines with `--`; then pull keys+defaults. 
...
> ***cli(`t` :tab) :tab*** :large_blue_circle:  For keys in `t`, look for updates on command-line. 
...
> ***on(`opts` :tab, `tests` :[fun])*** :large_blue_circle:  Run some tests.
...
> ***obj(`name` :str, `fun` :fun):object*** :large_blue_circle:  Return a klass `name` with constructor `fun`.
