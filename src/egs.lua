...
local as, is, lines = _.as, _.is, _.lines
...
-- Every new column goes into `all`.
-- Also, for any column that we
-- we are not ignoring, then that also gets added to (a) either
-- the list of `x` 
-- independent columns or `y` dependent columns; and
-- (b) maybe, the `klass` slot.
...
function Nominal:new(at,name)
  ...
  return as({at=at,name=name,n=0,has={}},Nominal) end

function Nominal.add(i,x)
  if x ~= "?" then i.n =i.n+1; i.has[x] = 1 + (i.has[x] or 0) end 
  return x end
-- ## Ratio
