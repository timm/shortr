...
-- __NUM:bin(x:num):num__   
-- NUMs get discretized to bins of size `(hi - lo)/THE.bins`.
function NUM.bin(i,x,   b)     
  if i.lo==i.hi then return 1 end
...
-- __SYM:score(want:any, wants:int, donts:init):float__    
-- SYMs get discretized to themselves.
...
-- tasks, data needs to be __normalized__ in which case -- we need to
-- know the space `of` data that holds this data.
...
-- __ROWS(src:str | tab):ROWS__   
-- Load in examples from a file string, or a list or rows.
...
function RANGES.new(i,col,klass, bests,rests) 
  i.out={}
  local ranges,n = {}, 0
  for label,rows in pairs{bests,rests} do -- for each set..
    n = n + #rows
    for _,row in pairs(rows) do   -- for each row...
      local v = row.cells[col.at] 
      if v ~= "?" then            -- count how often we see some value
        local r = col:bin(v)      -- accumulated into a few bins
        ranges[r] = -- This idiom means "ranges[x]" exists, and is stored in "out".
          ranges[r] or push(i.out,RANGE(v, v, klass(col.at,col.txt)))
        ranges[r]:add(v,label) end end end   -- do the counting
  table.sort(i.out,lt("xlo"))
  i.out = col.is=="NUM" and i:xpand(i:merge(i.out, n^THE.min)) or i.out 
  i.out = #i.out < 2 and {} or i.out end -- less than 2 ranges? then no splits found!

-- For numerics, **xpand** the ranges to cover the whole number line.
function RANGES:xpand(t)  
...
function RANGES.merge(i,b4,min,      t,j,a,b,c)
  t,j = {},1
  while j <= #b4 do 
    a, b = b4[j], b4[j+1]
    if b then 
       c = i:merged(a.ys, b.ys, min) -- merge small and/or complex bins
       if c then 
         j = j + 1
         a = RANGE(a.xlo, b.xhi, c) end end
    t[#t+1] = a
    j = j + 1 end
  return #b4 == #t and t or i:merge(t,min) end -- and maybe loop
...
function RANGES:merged(x,y,min,      z)
  z = x:merge(y)
